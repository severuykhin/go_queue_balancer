...
```
export PATH=/home/linuxbrew/.linuxbrew/Cellar/nats/0.0.28/bin:$PATH
```
...
1. Проблема - в монитор консьюмере запускается несколько горутин для сообщества если сообщения идут непрерывным потоком. 
Решение 1 - запускать монитор консьюмер только после инициализации сообществ
